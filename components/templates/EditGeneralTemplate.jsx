...
import BackgroundTab from "@/components/templates/tabs/general/BackgroundTab";
import OtherTabEdit from "@/components/templates/tabs/general/OtherTabEdit";
import SelectDialog from "@/components/general/SelectDialog";
import NavigationTab from "@/components/templates/tabs/general/NavigationTab";
import TabsGeneralButtons from "@/components/templates/general/TabsGeneralButtons";


function EditGeneralTemplate({ userTemplateList }) {
            ...
            {isDialogOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
                    <div className="bg-gradient-to-b from-main_background to-[#070707] border border-gray-700 sm:rounded-2xl max-sm:px-3 p-6 shadow-2xl max-sm:min-w-[100vw] sm:w-full max-w-7xl animate-fadeIn max-sm:max-h-[100vh] max-h-[90vh] flex flex-col">
                        ...
                        {/* Tabs */}
                        <TabsGeneralButtons activeTab={activeTab} setActiveTab={setActiveTab} isLoading={isLoading} />
                        ...
                        <div className={`overflow-y-auto flex-1 p-0.5 mb-4 ${isLoading ? 'pointer-events-none opacity-60' : ''}`} aria-disabled={isLoading}>
                            ...
                            {activeTab === 'navigation' && (
                                <NavigationTab settings={settings} setSettings={setSettings} handleRestore={handleRestore} />
                            )}

                            {/* Other Tab */}
                            {activeTab === 'other' && (
                                <OtherTabEdit settings={settings} setSettings={setSettings} handleRestore={handleRestore} templateName={templateName} setTemplateName={setTemplateName} originalTemplateName={originalTemplateName} />
                            )}
                        </div>
