...
async function scrapeFreeWebNovelChapter(page, url) {
    ...
    return page.evaluate(() => {
        ...
        const cleanupPatterns = [
            ...
            /[^\s<>]*?[fƒḟ́]?r[^\s<>]*?(?:e|е|ε|е́|ё|ē|)?[^\s<>]*?(?:e|е|ε|е́|ё|ē|)[^\s<>]*?(?:w|ω|ѡ|ԝ|)[^\s<>]*?(?:e|е|ε|е́|ё|ē|)[^\s<>]*?b[^\s<>]*?(?:n|ɳ|ո|ռ|)[^\s<>]*?(?:o|о|ο|օ|)[^\s<>]*?(?:v|ν|ṿ|ṽ|ѵ|)[^\s<>]*?(?:e|е|ε|е́|ё|ē|)[^\s<>]*?l[^\s<>]*?(?:\.|\․|。|｡|)?[^\s<>]*?(?:c|с|ϲ|ƈ|)[^\s<>]*?(?:o|о|ο|օ|)[^\s<>]*?(?:m|м|ṃ|ṁ|๓|)[^\s<>]*?(?=\s*<\/p>|\s*$)/gi,
