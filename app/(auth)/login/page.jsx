...
export default function Login() {
    ...
    return (
        <div className={"max-w-md mx-auto max-sm:my-32 my-24 p-6 bg-black/40 border-2 border-secondary/10 shadow-lg rounded-lg"}>
            <h1 className={"max-sm:text-2xl text-3xl font-bold text-center mb-6 text-secondary link_outline select-none"}>Login</h1>
            <form onSubmit={handleSubmit}>
                ...
                >
                    {isLoading ? <CircularProgress sx={{color: "#FAFAFA"}} size={28} thickness={8} /> : 'Login'}
                </button>
            </form>
            <div className={"mt-6 text-center"}>
                {isLoading ? (
                    ...
                    <Link href={"/register"} className={"text-blue-500 text-sm hover:underline select-none"}>
                        Don&apos;t have an account? Register
                    </Link>
                )}
            </div>
