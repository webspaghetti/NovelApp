const sourceConfig = {
    freewebnovel: {
        ...
        chapter_scraper: {
            ...
            watermarkPatterns: [
                /[^\s<>]*?(?:[fƒḟ́𝕗])?[^\s<>]*?(?:r|г|𝐫)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖)[^\s<>]*?(?:w|ω|ѡ|ԝ|𝕨)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖)[^\s<>]*?[^\s<>]*?(?:b|ɓ|𝗯)[^\s<>]*?(?:n|ɳ|ո|ռ|𝚗)[^\s<>]*?(?:o|о|ο|օ|σ|𝕠)[^\s<>]*?(?:v|ν|ṿ|ṽ|ѵ|𝘃)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖)[^\s<>]*?(?:l|𝚕)[^\s<>]*?(?:\.|\․|。|｡)?[^\s<>]*?(?:c|с|ϲ|ƈ|𝐜)[^\s<>]*?(?:o|о|ο|օ|σ|𝗼)[^\s<>]*?(?:m|м|ṃ|ṁ|๓|𝚖)[^\s<>]*?(?=\s*<\/p>|\s*$)/gi,
            ]
        }
    },
    lightnovelworld: {
