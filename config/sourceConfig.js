const sourceConfig = {
    freewebnovel: {
        ...
        chapter_scraper: {
            ...
            watermarkPatterns: [
                /[^\s<>]*?(?:f|ƒ|ḟ|𝕗|𝐟|ꜰ|ｆ|𝒻|𝚏|ϝ|ⅎ|ℱ|𝓯|𝔣|𝖋|ғ|⨍)?[^\s<>]*?(?:r|г|𝐫|ꞃ|ř|ｒ|ṙ|𝘳|𝚛|ʀ|ɾ|ɽ|𝓇|𝓻|𝔯|𝖗|ŗ|𝕣)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ|𝚎|𝗲|ə|ɘ|ҽ|𝓮|𝔢|𝖊|ė|ě|ę|έ)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ|𝚎|𝗲|ə|ɘ|ҽ|𝓮|𝔢|𝖊|ė|ě|ę|έ)[^\s<>]*?(?:w|ω|ѡ|ԝ|𝕨|𝗐|𝐰|ᴡ|ŵ|ｗ|𝘸|ʍ|₩|𝓌|𝔀|𝔴|𝖜)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ|𝚎|𝗲|ə|ɘ|ҽ|𝓮|𝔢|𝖊|ė|ě|ę|έ)[^\s<>]*?[^\s<>]*?(?:b|ɓ|𝐛|𝗯|𝕓|𝒷|ｂ|β|ʙ|𝓫|𝔟|𝖇|ḃ|ḅ)[^\s<>]*?(?:n|ɳ|ո|ռ|𝐧|𝚗|ｎ|𝘯|𝕟|𝗻|ɴ|ñ|ή|η|𝓃|𝓷|𝔫|𝖓|ń|ň|ņ)[^\s<>]*?(?:o|о|ο|օ|σ|ℴ|𝕠|𝗼|𝐨|ｏ|𝘰|𝑜|𝚘|0|ø|ö|ó|ò|ô|𝒪|𝓞|𝔒|𝕆|ⲟ|ʘ|º|°|٥|๐)[^\s<>]*?(?:v|ν|ṿ|ṽ|ѵ|𝐯|𝘃|ｖ|𝑣|𝚟|∨|⋁|𝒱|𝓥|𝕍|ᴠ|ѷ)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ|𝚎|𝗲|ə|ɘ|ҽ|𝓮|𝔢|𝖊|ė|ě|ę|έ)[^\s<>]*?(?:l|𝚕|𝐥|ｌ|ӏ|׀|l̥|𝑙|𝗹|ł|ľ|ļ|ℒ|ℓ|ⅼ|𝓁|𝓵|𝔩|𝖑|ʟ|ᒪ|ן|ı|1)[^\s<>]*?(?:\.|\․|。|｡|·|‧|⋅|•|●|∙|⸱)?[^\s<>]*?(?:c|с|ϲ|ƈ|𝐜|ｃ|𝘤|𝕔|ϲ|ⲥ|ç|ć|č|ℂ|ℭ|𝒞|𝓒|𝓬|𝔠|𝖈|ͻ|⊂|¢|𝗰)[^\s<>]*?(?:o|о|ο|օ|σ|ℴ|𝕠|𝗼|𝐨|ｏ|𝘰|𝑜|𝚘|0|ø|ö|ó|ò|ô|𝒪|𝓞|𝔒|𝕆|ⲟ|ʘ|º|°|٥|๐)[^\s<>]*?(?:m|м|ṃ|ṁ|๓|𝐦|𝚖|ｍ|𝘮|𝗺|𝕞|μ|ᴍ|ṁ|ɱ|ḿ|𝓜|𝓂|𝓶|𝔪|𝖒)[^\s<>]*?(?=\s*<\/p>|\s*$)/gi,
            ]
        }
    },
    lightnovelworld: {
