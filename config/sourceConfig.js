const sourceConfig = {
    freewebnovel: {
        ...
        chapter_scraper: {
            ...
            watermarkPatterns: [
                /[^\s<>]*?(?:f|ƒ|ḟ|𝕗|𝐟|ꜰ|ｆ|𝒻|𝚏)?[^\s<>]*?(?:r|г|𝐫|ꞃ|ř|ｒ|ṙ|𝘳)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ|𝚎|𝗲)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ|𝚎|𝗲)[^\s<>]*?(?:w|ω|ѡ|ԝ|𝕨|𝗐|𝐰|ᴡ|ŵ|ｗ|𝘸)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ|𝚎|𝗲)[^\s<>]*?[^\s<>]*?(?:b|ɓ|𝐛|𝗯|𝕓|𝒷|ｂ)[^\s<>]*?(?:n|ɳ|ո|ռ|𝐧|𝚗|ｎ|𝘯|𝕟)[^\s<>]*?(?:o|о|ο|օ|σ|ℴ|𝕠|𝗼|𝐨|ｏ|𝘰|𝑜|𝚘)[^\s<>]*?(?:v|ν|ṿ|ṽ|ѵ|𝐯|𝘃|ｖ|𝑣)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ|𝚎|𝗲)[^\s<>]*?(?:l|𝚕|𝐥|ｌ|ӏ|׀|l̥|𝑙|𝗹)[^\s<>]*?(?:\.|\․|。|｡)?[^\s<>]*?(?:c|с|ϲ|ƈ|𝐜|ｃ|𝘤|𝕔)[^\s<>]*?(?:o|о|ο|օ|σ|ℴ|𝕠|𝗼|𝐨|ｏ|𝘰|𝑜|𝚘)[^\s<>]*?(?:m|м|ṃ|ṁ|๓|𝐦|𝚖|ｍ|𝘮|𝗺)[^\s<>]*?(?=\s*<\/p>|\s*$)/gi,
            ]
        }
    },
    lightnovelworld: {
