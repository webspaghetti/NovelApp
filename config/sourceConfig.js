const sourceConfig = {
    freewebnovel: {
        ...
        chapter_scraper: {
            ...
            watermarkPatterns: [
                /[^\s<>]*?(?:f|ƒ|ḟ|𝕗|𝐟|ꜰ|ｆ)?[^\s<>]*?(?:r|г|𝐫|ꞃ|ř|ｒ|ṙ)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ)[^\s<>]*?(?:w|ω|ѡ|ԝ|𝕨|𝗐|𝐰|ᴡ|ŵ|ｗ)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ)[^\s<>]*?[^\s<>]*?(?:b|ɓ|𝐛|𝗯|𝕓|𝒷|ｂ)[^\s<>]*?(?:n|ɳ|ո|ռ|𝐧|𝚗|ｎ)[^\s<>]*?(?:o|о|ο|օ|σ|ℴ|𝕠|𝗼|𝐨|ｏ)[^\s<>]*?(?:v|ν|ṿ|ṽ|ѵ|𝐯|𝘃|ｖ)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ)[^\s<>]*?(?:l|𝚕|𝐥|ｌ|ӏ|׀|l̥)[^\s<>]*?(?:\.|\․|。|｡)?[^\s<>]*?(?:c|с|ϲ|ƈ|𝐜|ｃ)[^\s<>]*?(?:o|о|ο|օ|σ|ℴ|𝕠|𝗼|𝐨|ｏ)[^\s<>]*?(?:m|м|ṃ|ṁ|๓|𝐦|𝚖|ｍ)[^\s<>]*?(?=\s*<\/p>|\s*$)/gi,
            ]
        }
    },
    lightnovelworld: {
