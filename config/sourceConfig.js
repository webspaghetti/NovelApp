const sourceConfig = {
    freewebnovel: {
        ...
        chapter_scraper: {
            ...
            watermarkPatterns: [
                /[^\s<>]*?(?:f|ƒ|ḟ|𝕗|𝐟|ꜰ|ｆ|𝒻)?[^\s<>]*?(?:r|г|𝐫|ꞃ|ř|ｒ|ṙ|𝘳)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ)[^\s<>]*?(?:w|ω|ѡ|ԝ|𝕨|𝗐|𝐰|ᴡ|ŵ|ｗ|𝘸)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ)[^\s<>]*?[^\s<>]*?(?:b|ɓ|𝐛|𝗯|𝕓|𝒷|ｂ)[^\s<>]*?(?:n|ɳ|ո|ռ|𝐧|𝚗|ｎ|𝘯)[^\s<>]*?(?:o|о|ο|օ|σ|ℴ|𝕠|𝗼|𝐨|ｏ|𝘰|𝑜)[^\s<>]*?(?:v|ν|ṿ|ṽ|ѵ|𝐯|𝘃|ｖ|𝑣)[^\s<>]*?(?:e|е|ε|е́|ё|ē|𝐞|𝕖|℮|ẹ|ｅ|𝘦|ℯ)[^\s<>]*?(?:l|𝚕|𝐥|ｌ|ӏ|׀|l̥|𝑙)[^\s<>]*?(?:\.|\․|。|｡)?[^\s<>]*?(?:c|с|ϲ|ƈ|𝐜|ｃ|𝘤)[^\s<>]*?(?:o|о|ο|օ|σ|ℴ|𝕠|𝗼|𝐨|ｏ|𝘰|𝑜)[^\s<>]*?(?:m|м|ṃ|ṁ|๓|𝐦|𝚖|ｍ|𝘮)[^\s<>]*?(?=\s*<\/p>|\s*$)/gi,
            ]
        }
    },
    lightnovelworld: {
